...
namespace FreeKi
{
	public class Program
	{
		...
		{
			FreeKiOptions? options = FreeKiOptions.Parse(args);
			if (options != null)
			{
		...
		static private async Task Run(FreeKiOptions o)
		{
			...
			// PosixSignalRegistration is AOT-safe, unlike hooking AssemblyLoadContext.Unloading.
			using PosixSignalRegistration sigTerm = PosixSignalRegistration.Create(PosixSignal.SIGTERM, (PosixSignalContext context) =>
				{
					if (sigIntRecvd==false)  // don't process this if control-c happened
					{
						...
						context.Cancel = true;  // we handle shutdown ourselves; don't let the runtime terminate immediately
						tokenSrc.Cancel();
					}
				});
			...
			try
			{
				...
				await Task.Delay(Timeout.Infinite, tokenSrc.Token);  // block here until the cancellation token triggers (TaskCanceledException, caught below as flow control).  Note, if the server decides to shut itself down, IT CANCELS THIS TOKEN.  So this is the perfect way to wait.
			}
