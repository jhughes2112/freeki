...
namespace DataCollection
{
    ...
    public class DataCollectionFake : IDataCollection
    {
		...
		private ThreadSafeDictionary<string, ValueDesc> _histograms    = new ThreadSafeDictionary<string, ValueDesc>();
		...
		public void CreateHistogram(string histogramName, string description, double[] bucketUpperBounds)
		{
			...
			if (_histograms.Add(histogramName, new ValueDesc(0, description))==false)
				throw new Exception($"DataCollection.CreateHistogram logic error {histogramName} already exists.");
		}
		...
		public void ObserveHistogram(string histogramName, double value)
		{
			...
			Interlocked.Increment(ref vd.value);  // fake just tracks the number of observations
		}
