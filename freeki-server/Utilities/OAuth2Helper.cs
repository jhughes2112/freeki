				...
				for (int i=0; i<kRetries; i++)
				{
					using (HttpClient client = new HttpClient())
					{
						...
						try
						{
							...
							config = JsonSerializationRouter.Deserialize<OAuthConfiguration>(discoveryResponse);
							break;
						}
						catch (Exception e)
						{
					...
					if (_publicKeys.TryGetValue(config.jwks_uri, out keys)==false)
					{
						...
						using (HttpClient client = new HttpClient())
						{
							try
							{
								...
								KeySet? jwksData = JsonSerializationRouter.Deserialize<KeySet>(jwksResponse);
